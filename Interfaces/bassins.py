...
from Calcul.Calculs_portance import tracer_bassins_deflexion, DEFLEXION_COLS
...
class BassinsTab(OngletCanvasSeul):
    ...
    def _build_controls(self):
        self._combos = {}
        widgets = []
        for label, key in [("Appareil", "appareil"), ("Point", "point"),
    ...
    def _tracer(self):
        ...
        if self.chk_moyenne.isChecked():
            grp    = [c for c in ['appareil', 'point', 'niveau_effort', 'passage'] if c in df.columns]
        ...
        tracer_bassins_deflexion(
            df, ax=ax,
            titre=(f"Bassin — {labels['appareil']} | {labels['point']} | "
